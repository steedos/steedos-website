...
- *置换表达式:* `text/number/currency/percent/date/datetime`
...
- ■ 传入的参数不支持布尔类型值，且传入的两个参数值类型应该一样，否则公式将直接报错。
- ■ 如果您对数值字段使用该函数，则字段没有值且配置将空白字段处理为空白时，该函数才返回第二个参数中的指定的置换表达式值，如果字段没有值且配置将空白字段处理为0时，则按0值返回而不是返回第二个参数中的指定表达式值。
