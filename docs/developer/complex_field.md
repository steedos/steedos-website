...
对于自动计算值的自定义公式字段，可以设置其字段类型为公式字段 formula， 并设置返回值类型data_type。如此设置后，这个字段无需手动输入，而由系统自动计算得出。
...
如上，汇总summary_object设置为quote_line_item，子表“报价行项目”对象，即累计汇总是基于子表“报价行项目”对象进行计算。
...
如上，汇总summary_field设置为"total_price"，子表“报价行项目”对象的字段“总价”，即汇总字段是基于子表“报价行项目”中的字段“总价”进行汇总。